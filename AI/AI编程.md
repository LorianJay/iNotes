...
#### 1.4 AI中转站  
1.介绍  
有些人不方便用国外的API,于是就需要代购,那么中转站就把各大API封装好给你用,顺便赚个差价和服务费  
这东西能火起来,主要也是因为国外的大模型有支付和网络的门槛,中转站就解决了这些问题,而且它还可以帮助你管理所有模型的API KEY而不需要你手动管理  
目前最知名的中转站就是[OpenRouter](https://openrouter.ai/)  

2.中转站的便利  
目前大多数中转站都有一个共同的设计,就是对外"伪装"成OpenAI的接口格式,这样用户只需要改2个参数就能接入中转站,一行业务代码都不用动,用起来十分方便  
```python
# python
client = OpenAI(
    api_key="sk-relay-xxx",       # 改成中转站的Key
    base_url="http://你的中转站/v1" # 改成中转站的地址
)
```

3.中转站的危险  
问题显而易见就是,中转站会卖假货因为用户的所有请求都经过中转站,这一层对⁡用户来说完全是个黑盒,常见的黑心操作有:模型掉包、虚报Token消耗、缓存套利(AI缓存命中时消耗很低,但是中转站把这部分吃掉了)、数据收割(贩卖用户的个人信息)  

4.开源中转站项目  
*提示:这些开源项目的意义在于,可以帮助你管理各种模型的API Key,并提供统一的接入方式,所以与其叫中转站项目,不如叫API Key管理项目*  
* [one-api](https://github.com/songquanpeng/one-api)
* [new-api](https://github.com/QuantumNous/new-api)  
* [sub2api](https://github.com/Wei-Shaw/sub2api)
  能把Claude、OpenAI的网页版订阅账号逆向转化成标准⁡的API接口,相当于多个人拼​车共享一个Plus会员,一个账号的额度分给好几个人用
* [metapi](https://github.com/cita-777/metapi)
  号称中转站的中‍转站,把你在多个one-api/new-api/⁡sub2api上注册的账号聚​合成一个统一网关,基于成本、余额、使用率三维加权来分配流量
* [all-api-hub](https://github.com/qixing-jk/all-api-hub)
  这是一个浏览器扩展‍,专门用来管理你在各个中转站上的账号,装上之后,可以在一个面板里看到⁡所有中转站的余额和用量,还支持自动​签到、价格横向对比,对于同时用好几个中转站的人来说挺方便的
