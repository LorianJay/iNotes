...
* GPT-4o:128K Token
* GPT-5.5:1M Token
* Claude Opus 4.8:标准200K Token,支持扩展到1M Token
* Gemini 3.1 Pro:1M Token
* DeepSeek-V4-Pro:1M Token
...
15.6 Token缓存  
大模型在处理你的提示词时,需要做大量的计算,如果你连续多次对话,很多内容(比如系统提⁡示词、引用的代码文件)是重复的,每次都重新计算就太​浪费了;缓存机制会把这些中间计算结果保存下来,下次遇到相同的前缀内容就直接复用,不仅更快,还更便宜  
缓存相关的 Token 分为两种:  
* 缓存写入Token:AI第一次处理你的上下文时,会把计算结果存起来,比普通输入略贵一点
* 缓存读取Token:后续再用相同的上下文时,直接复用缓存,价格甚至可以低到普通输入的1/10,非常便宜
...
17.Training/Inference(训练/推理)  
运行在个人设备上的模型,例如3B-9B的模型;会分为两个步骤,首先是各大厂商先训练Training模型,这个过程需要上千张显卡花几个月时间进行训练;厂商训练好之后会发布一个几GB大小的权重文件,把该文件下载到手机或电脑上运行(称为Inference)推理过程  
...
21.Fine-tuning(模型微调)  
微调是在已‍有模型的基础上,用特定领域的数据继续⁡训练,让模型在某个​领域表现更好  
比如,你可‍以用大量的医学资料微调一个模型,让它成为医学⁡专家;或者用你公​司的代码库微调,让它更了解你的项目风格  
对于普通用户来说‍,微调成本较高,一般不需要自己做,直接使用现成的模型就够了,不过⁡,很多大模型应用开发平台(比如阿​里云百炼、火山引擎等)都<font color="#FF00FF">提供了模型微调的能</font>力,降低了微调的门槛  

22.模型蒸馏(Knowledge Distil⁡lation)  
模型蒸馏是一种​把大模型的知识"压缩"到小模型里的技术,蒸馏的核心在于<font color="#00FF00">软标签</font>,举个例子,让大模型识别一张动物图片,它不会只说这是狗,而是会给出一组概率:狗92%、猫5%、狼3%.那个5%⁡的猫,其实包含了珍贵的信息,说明这张图里的动物和猫有​某些相似特征,小模型通过学习这些概率分布,能学到类别之间的微妙关系,效果比只看"对或错"的硬标签好得多  
微调是在已有‍模型基础上用特定数据继续训练,让它在某个领域⁡更专业;蒸馏是把大模型​的知识转移到小模型里,让小模型更轻量、更便宜  

23.智能体编排(Orc‍hestration)  
编排(Orc‍hestration)是指协调和管理多个A⁡I智能体或AI任​务的过程,确保它们按正确的顺序和方式工作  
如果说多智能‍体协作关注的是"有哪些角色参与",那编排关⁡注的是"谁先干、谁后​干、结果怎么汇总",它是多智能体系统的指挥中枢  
就像乐队指‍挥一样,编排器决定哪个智能体在什么时⁡候做什么事情、如何​传递信息、如何汇总结果  

24.Hooks  
Hooks(钩子‍)是AI编程工具中的一种自动化触发器,当AI完成某个⁡动作(比如生成代码、提交代码、​运行命令)时,Hook会自动执行你预设的脚本或检查流程  

25.jev  
jev是一个模型,本质就是把大模型一些不擅长的领域单拎出来,比如大模型不擅长做计算,那就把计算逻辑交给计算器,这样就营造出一种比大模型又快又准又便宜的假象,还有一类问题大模型也不擅长就是<font color="#00FF00">判断</font>问题,尤其是一次性做多组判断  
比如输入给大模型的文本是一个工单"你们的支付服务咋回事?一个小时了还没恢复!用户都投诉到老板那里了!"此时希望大模型判断如下三个问题,这个工单的类型是什么?(bug/建议/邀约)、这个工单归属于哪个部门?(支付/会员/商业化)、紧急程度是多少(低/中/高),并要求大模型按照下面的格式进行回答  
类型:xxx 部门:xxx 程度:xxx  
所以jev需要两个输入,一个是状态(就是想让大模型分析的上下文信息)、一个是问题,然后jev会直接返回结构化的答案,jev支持回答三种类型的问题,判断题、选择题、打分题  
这个问题可以交给大模型解决,但是这就把生成式模型强行用成了一个分类模型,并且大模型在做这个问题的时候是按token输出的,jev的优化思路就是把问题拆分成三次对话,然后并行地把问题送给大模型,jev的底层还是transform架构,只是在最后输出的时候会针对这些美剧类型的token(bug/建议/邀约)让其分别经过一个打分器(小神经网络),这个打分器会做专项训练,在jev中被称为全新的强化学习训练RLCD,代替传统的RLHF  
所以jev这个模型即希望保留大模型的泛化能力,又希望把一些功能窄化到一个特定的领域,所以它就做了一个折中,但这样做的代价就是,输入给大模型的提示词必须按照要求的格式输入,而且模型的输出也是比较固定的,总体来看<font color="#00FF00">jev是非常适合判断类型的场景的</font>  
...
## 1.5 大模型排行  
1.数据来源  
[DataLearnerAI](https://www.datalearner.com/leaderboards)  
![大模型排行](resources/AIProgram/4.png)  

## 1.6 AI Infra  
1.介绍  
AI Infra(Infrastructure)也就是人工智能基础设施,用于支撑AI模型从训练到使用的全套底层软硬件系统  

2.岗位职责
具体职责包括,让多张显卡同时工作、最大化发挥算力等都是infra要解决的问题  
* 分布式训练框架开发:让大量显卡协同训练大模型,解决通信与内存切分问题
* AI推理引擎开发:极致性能压榨,用最少的算力实现最快的响应速度
* 高性能算子/kernel(内核)开发:深入GPU硬件架构,手写底层计算逻辑,逼近硬件极限
* AI算力集群与MLOps:管理万卡GPU集群,保障AI系统稳定性与资源调度

3.门槛分析  
需要扎实的计算机底层功底,92本硕学历,需要<font color="#00FF00">垂直实习经历</font>,工程能力突出  

4.学习路线  
* 基础通用技能:C++、操作系统原理和实战、网络、基本数据结构与算法、设计模式、py基础
* AI infra通用技术栈:深度学习基础、并行计算入门
* 方向A:算子开发工程师(GPU算子优化方向)
* 方向B:大模型推理优化工程师(推理引擎开发方向)
* 方向C:分布式训练框架工程师(大模型训练系统方向)
* <font color="#FF00FF">方向D:AI算力调度、云原生AI工程师</font> 

5.普通学历入行  
找第一段中小厂实习积累工作经验,之后找一二线ai infra组,因为除了工作经验之外并没有什么优势,所以一定要在技术深度下功夫  
普通人进AI infra最容易的就是推理,偏向工程,研究部分很少,缺点是推理的HC少  
项目的话就因人而异,自已做的基本都是玩具项目,infra很吃大规模分布式训练和多卡并行推理和各种生产时的分析,应对这些问题的实际经验是最珍贵的  
...
## 2.7 计算机视觉CV  
1.学习路线
* [鱼皮的计算机视觉学习路线](https://www.codefather.cn/course/1789189862986850306/section/1990748555425394689)  
* 程序员亮亮的学习路线:C++起步->Linux工程起步->OpenCV与传统视觉->Python、PyTorch、深度视觉->ROS2、三维视觉与点云、C++部署与感知系统工程


2.工作相关  
* <font color="#00FF00">视觉算法岗</font>不太好就业,纯算法研究岗位少竞争激烈,要求一般是顶会论文、竞赛成绩、高质量实习和开源项目,岗位主要集中在大厂、AI独角兽和研究院->学计算机视觉纯纯脑残行为  
* CV应用工程师,本硕都能投递,大量招人,工作内容一般是拿成熟的模型比如YOLO做业务适配,解决真实场景的问题 
* 机器视觉/嵌入式视觉工程师,制造业刚需,岗位最多,传统视觉+深度学习结合,做工业检测、测量、3D视觉,相对来说好入行一点

3.可投岗位  
* 消费级视觉、工业机器视觉、车载视觉、医疗视觉、算法/芯片  
* 工业机器视觉(推荐),工业质检、测量、读码、机器人引导
* 车载视觉,多目摄像头、深度相机->相对来说卷一点(因为计算机视觉主要就是车企在招聘,现在比较饱和了)
* 医疗视觉,医疗图像分割、辅助诊断、手术导航->不是特别推荐,没什么钱
* 视觉算法和芯片(天花板),薪资高、岗位竞争激烈
* 具身机器人(推荐),SLAM、VLA工程师->搞应用而不是算法

4.岗位推荐  
![岗位推荐](resources/AIProgram/5.png)  

## 2.8 FDE  
1.解释  
FDE的意思就是前沿部署工程师,简单理解就是套外派到客户侧提供从需求分析到开发测试部署可落地并且可以反馈持续产品迭代的解决方案  
因为很多公司提供的AI产品并不是部署完就结束了,可能中间还涉及很多调优还要给客户定制化开发类似于RAG增强的一些服务,还需要持续挖掘需求、客户管理、商务对接、市场营销,跟驻场开发的区别是,驻场开发不会涉及到太多的产品设计,而FDE是需要更多地涉及到产品需求分析跟客户管理  

2.本体论  
AI回答一些通识问题非常厉害,但是要和某一家公司的业务具体结合时,经常就会出现各种问题,这是因为AI不懂你的这家公司,他不理解你的业务模式、公司内部的话术,甚至对特定名词不同公司的理解也是不一样的,比如"高价值用户"  
所以为了能够让AI更懂你的公司,就出现了直接让AI接入数据库,或接入RAG,但这并不能从根本上解决问题  
所以此时就出现了<font color="#FF00FF">本体论</font>的概念,简单来说本体就是给AI的一本入职手册,本体就是一本手册只不过是写给机器的手册,手册包含四个方面,对象、客户、合同、设备以及他们之间的关系  
<font color="#00FF00">所以本体=对象+关系+动作+规则</font>  
语义层和本体的区别,语义层解决全公司的口径统一问题,例如什么是活跃用户、销售额怎么算,所有的系统和AI都会按照该定义去理解同一个词,所以它的本质是<font color="#00FF00">解释一个词</font>,语义层是公司的统一词典,而本体是员工手册需要解决遇到某个情况怎么解决,能干什么、不能干什么  
<font color="#00FF00">所以最适合编写本体的人就是FDE</font>  
